                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Dismissible(
                          ...
                          onDismissed: (direction) {
                            Provider.of<ClienteController>(context, listen: false).deletarCliente(cliente.id);
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('Cliente "${cliente.nome}" excluído.')),
                            );
                          },
                          child: Card(
                            margin: const EdgeInsets.symmetric(vertical: 4, horizontal: 4),
                            child: ListTile(
                              leading: Icon(cliente.tipoPessoa == TipoPessoa.fisica ? Icons.person : Icons.business, color: Theme.of(context).colorScheme.primary, size: 32),
                              title: Text(cliente.nome, style: const TextStyle(fontWeight: FontWeight.bold)),
                              trailing: const Icon(Icons.arrow_forward_ios, size: 16),
                              onTap: () {
                                Navigator.pushNamed(context, NovoCliente.routeName, arguments: cliente);
                              },
                            ),
                          ),
                        );
