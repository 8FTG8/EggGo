  ...
  Widget build(BuildContext context) {
    ...
    return Dismissible(
      ...
      background: Container(
        ...
        decoration: BoxDecoration(color: colorScheme.primary.withAlpha(204), borderRadius: BorderRadius.circular(12)),
                ...
                Autocomplete<Produto>(
                  ...
                  optionsBuilder: (TextEditingValue textEditingValue) {
                    ...
                    return widget.allProducts.where((Produto option) {
                      ...
                      return option.nome.toLowerCase().contains(input) || option.codigo.toLowerCase().contains(input);
                    });
                  },
                  ...
                  fieldViewBuilder: (context, controller, focusNode, onFieldSubmitted) {
                    return TextFormField(
                      ...
                      focusNode: focusNode,
                      decoration: const InputDecoration(labelText: '* Nome do produto', isDense: true),
                      validator: (value) {
                  ...
                  children: [
                    Expanded(
                      ...
                      child: TextFormField(
                        ...
                        validator: (v) => (int.tryParse(v ?? '0') ?? 0) <= 0 ? 'Quantidade inválida!' : null,
                      ),
                    ),
                    ...
                    Expanded(
                      flex: 2,
                      child: TextFormField(
                        decoration: const InputDecoration(labelText: '* Preço Unitário', isDense: true),
                        keyboardType: const TextInputType.numberWithOptions(decimal: true),
                        onChanged: (value) {
                          widget.produtoForm.precoUnitario = double.tryParse(value.replaceAll(',', '.')) ?? 0.0;
                          widget.onChanged(widget.produtoForm);
                        },
                        validator: (v) => (double.tryParse(v?.replaceAll(',', '.') ?? '0.0') ?? 0.0) <= 0 ? 'Preço inválido!' : null,
                      ),
                    ),
                  ],
                ),
